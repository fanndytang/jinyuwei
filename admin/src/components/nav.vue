    ...
    export default {
        ...
        methods: {
            getOneNav (callback) { //  判断同类型的是否已存在相同序号的链接
                ...
                if (!flag) {
                    self.$http.get('/api/nav/onenav?sort=' + self.ruleForm.sort + '&&type=' + self.type).then(data => {
            ...
            getNav () {
                let self = this
                self.$http.get('/api/nav/list?type=' + self.type).then(data => {
                    let list = JSON.parse(data.request.response).data
                    for (let key in list) {
